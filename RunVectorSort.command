...
# Default CSV in repo
CSV="eBid_Monthly_Sales.csv"

# Offer to choose a CSV via AppleScript dialog (if available)
CHOICE=$(osascript <<'APPLESCRIPT'
try
set theButton to button returned of (display dialog "Run with default CSV (eBid_Monthly_Sales.csv)?" buttons {"Choose CSV…","Default"} default button "Default")
